...
Windows 10 / 11（64 位）可直接安装，无需 Python、Node.js 或命令行。安装程序按当前用户安装，不要求管理员权限。

**[下载康迹 0.1.0 Windows 安装包](https://github.com/HugoYzz/Wellness-Trail/raw/main/releases/Kangji_0.1.0_x64-setup.exe)**（约 112 MB）

1. 下载 `Kangji_0.1.0_x64-setup.exe` 后双击安装。
2. 从开始菜单打开「康迹」，首次使用时设置 6–12 位数字 PIN。
3. 健康数据只保存在本机，不会包含在安装包里。

安装包尚未代码签名，Windows 可能会提示发布者未知。请确认文件来自本仓库后再继续安装。
